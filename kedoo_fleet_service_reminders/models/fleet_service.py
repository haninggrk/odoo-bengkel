...
class FleetVehicleLogServices(models.Model):
    ...
    next_service_date = fields.Date(
        ...
        help='Scheduled date for the next service. A reminder is sent in advance based on settings.',
    )
    ...
    def _build_reminder_payload(self):
        ...
        vehicle_brand = vehicle.model_id.brand_id.name if vehicle.model_id and vehicle.model_id.brand_id else ''
    ...
    def _send_evolution_reminder(self, payload, base_url, instance_name, api_key, country_code, template):
        ...
        if not number:
            _logger.warning('Skipped Evolution reminder for service %s because no phone/mobile found.', self.id)
            return
    ...
    def _send_service_reminder(self, trigger='cron', raise_on_error=False):
        ...
        provider = params.get_param('kedoo_fleet_service_reminders.provider', 'webhook')
        webhook_url = params.get_param('kedoo_fleet_service_reminders.webhook_url')
        evolution_base_url = params.get_param('kedoo_fleet_service_reminders.evolution_base_url')
        evolution_instance_name = params.get_param('kedoo_fleet_service_reminders.evolution_instance_name')
        evolution_api_key = params.get_param('kedoo_fleet_service_reminders.evolution_api_key')
        evolution_country_code = params.get_param('kedoo_fleet_service_reminders.evolution_country_code', '62')
        evolution_template = params.get_param('kedoo_fleet_service_reminders.evolution_message_template')
        ...
        if provider == 'webhook' and not webhook_url:
            if evolution_base_url and evolution_instance_name and evolution_api_key:
                ...
                msg = _(
                    'Reminder provider is Generic Webhook, but Webhook URL is empty. '
                    'Switch provider to Evolution API and fill required settings.'
                )
                if raise_on_error:
                    raise UserError(msg)
                _logger.warning(msg)
        ...
        try:
            ...
            if raise_on_error:
                raise
            _logger.error('Failed to send reminder for service %s via %s with user error.', self.id, provider)
            return False
        except Exception as exc:
            if raise_on_error:
                raise UserError(_('Failed to send reminder: %s') % exc)
            _logger.error('Failed to send reminder for service %s via %s: %s', self.id, provider, exc)
            return False
    ...
    def _cron_send_service_reminders(self):
        params = self.env['ir.config_parameter'].sudo()
        reminder_days = int(params.get_param('kedoo_fleet_service_reminders.days_before', 3) or 3)
        today = date.today()
